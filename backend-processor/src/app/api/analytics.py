...
async def analyze_products(
    ...
    with tracer.start_as_current_span("analytics_analyze_products") as span:
        ...
        try:
            ...
            top_products = sorted(
                products, key=lambda x: x.get("price", 0), reverse=True
            )[:5]
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to analyze products: {str(e)}"
            )


@router.get("/orders", response_model=OrderAnalytics)
async def analyze_orders(settings: Settings = Depends(get_settings)) -> OrderAnalytics:
    """
    ...
    with tracer.start_as_current_span("analytics_analyze_orders") as span:
        ...
        try:
            ...
            logger.error(
                "Order analytics failed", error=str(e), event="analytics_orders_error"
            )

            raise HTTPException(
                status_code=500, detail=f"Failed to analyze orders: {str(e)}"
            )
        ...
        try:
            ...
            with tracer.start_as_current_span(
                ...
                health_data, products, orders = await asyncio.gather(
                    health_task, products_task, orders_task, return_exceptions=True
                )
            ...
            product_analytics = ProductAnalytics(
                ...
                average_price=round(sum(product_prices) / len(product_prices), 2)
                if product_prices
                else 0,
                top_products=top_products,
                analysis_timestamp=datetime.now(),
            ...
            order_analytics = OrderAnalytics(
                ...
                average_order_value=round(order_revenue / len(orders), 2)
                if orders
                else 0,
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to generate system report: {str(e)}"
            )
...
async def process_product_batch(
    ...
    with tracer.start_as_current_span("analytics_process_batch") as span:
        ...
        try:
            ...
            # Fetch products
            products = await demo_client.get_products(
                category=category, limit=batch_size
            )
            ...
            for product in products:
                with tracer.start_as_current_span(
                    "analytics_process_single_product"
                ) as product_span:
                    ...
                    if detailed_product:
                        ...
                        total_processing_time += processing_result[
                            "processing_duration_ms"
                        ]

            span.set_attribute("analytics.products_processed", len(processed_products))
            span.set_attribute(
                "analytics.total_processing_time_ms", total_processing_time
            )
            ...
            result = {
                ...
                "average_processing_time_ms": total_processing_time
