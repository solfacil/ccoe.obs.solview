    ...
    with tracer.start_as_current_span("processor_order_workflow") as span:
        ...
        try:
            ...
            with tracer.start_as_current_span(
                ...
                current_order = next(
                    (o for o in orders if o.get("id") == order_id), None
                )
            ...
            if request.notify_customer:
                background_tasks.add_task(
                    send_order_notification, order_id, request.customer_email, settings
                )
...
async def enrich_products(
    request: ProductEnrichmentRequest, settings: Settings = Depends(get_settings)
) -> Dict[str, Any]:
    """
    ...
    with tracer.start_as_current_span("processor_enrich_products") as span:
        ...
        try:
            ...
            with tracer.start_as_current_span(
                "processor_parallel_product_fetch"
            ) as parallel_span:
                ...
                product_results = await asyncio.gather(
                    *product_tasks, return_exceptions=True
                )
            ...
            span.set_attribute(
                ...
                len(enriched_products) / len(request.product_ids),
            )
            ...
            result = {
                ...
                "success_rate": round(
                    len(enriched_products) / len(request.product_ids), 2
                ),
                "enrichment_type": request.enrichment_type,
                "processing_timestamp": datetime.now().isoformat(),
                "enriched_products": enriched_products,
            ...
            raise HTTPException(
                status_code=500, detail=f"Product enrichment failed: {str(e)}"
            )
...
async def send_order_notification(
    order_id: str, customer_email: str, settings: Settings
):
    """
