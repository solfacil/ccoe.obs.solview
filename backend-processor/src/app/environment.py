...
class Settings(BaseSettings):
    ...
    otel_exporter_endpoint: str = Field(
        default="http://otel-collector:4317", env="OTEL_EXPORTER_OTLP_ENDPOINT"
    )
    otel_exporter_protocol: str = Field(
        default="grpc", env="OTEL_EXPORTER_OTLP_PROTOCOL"
    )
    ...
    cors_origins: str = Field(
        default="http://localhost:3000,http://localhost:9090", env="CORS_ORIGINS"
    )
