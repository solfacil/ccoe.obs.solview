...
def test_metric_requests_type_and_increment():
    ...
    metric = core.METRIC_REQUESTS.labels(
...
def test_metric_responses_labels_and_increment():
    ...
    metric = core.METRIC_RESPONSES.labels(
        method="GET", path="/", status_code="200", service_name="solview-test"
    )
    metric.inc()
...
def test_metric_processing_time_observe():
    ...
    metric = core.METRIC_REQUESTS_PROCESSING_TIME.labels(
        method="GET", path="/", service_name="solview-test"
    )
