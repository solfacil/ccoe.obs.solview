...
class DataMasker:
    ...
    def mask_sensitive_data(self, text: str) -> str:
        ...
        text = re.sub(
            r"\b(\d{3})\d{6}(\d{2})\b", r"\1.XXX.XXX-\2", text
        )  # Without mask
        text = re.sub(
            r"\b(\d{3})\.\d{3}\.\d{3}-(\d{2})\b", r"\1.XXX.XXX-\2", text
        )  # With mask

        # CNPJ - Keep first 2 and last 2 digits
        text = re.sub(
            r"\b(\d{2})\d{10}(\d{2})\b", r"\1.XXX.XXX/XXXX-\2", text
        )  # Without mask
        text = re.sub(
            r"\b(\d{2})\.\d{3}\.\d{3}/\d{4}-(\d{2})\b", r"\1.XXX.XXX/XXXX-\2", text
        )  # With mask

        # Phone - Keep area code and last 4 digits
        text = re.sub(r"\b(\d{2})\d{4,5}(\d{4})\b", r"\1*****\2", text)  # Without mask
        text = re.sub(
            r"\b(\(\d{2}\))\s?\d{4,5}-\d{4}\b", r"\1 XXXXX-XXXX", text
        )  # With mask
    ...
    def sanitize_message(self, msg: Any) -> str:
        ...
        if isinstance(msg, Exception):
            ...
            msg = {
                key: self._convert_to_serializable(value) for key, value in msg.items()
            }
