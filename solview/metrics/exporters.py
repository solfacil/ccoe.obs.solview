...
class SolviewPrometheusMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        if not is_handled_path:
            _logger.debug(
                f"method={method} path={request.url.path} is_handled_path=False"
            )
            return await call_next(request)
        METRIC_REQUESTS_IN_PROGRESS.labels(
            method=method, path=path, service_name=self.service_name
        ).inc()
        ...
        try:
            ...
            METRIC_REQUESTS_PROCESSING_TIME.labels(
                method=method, path=path, service_name=self.service_name
            ).observe(after_time - before_time, **kwargs)
        finally:
